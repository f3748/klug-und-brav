...
const requiredHtml = ['id="status-bars"', 'id="mini-map"', 'id="map-overlay"', 'id="large-map"', 'id="room-scene"', 'id="narration"', 'prototype-note', 'pet-weimar', 'pet-sprite', 'weimar-idle.svg'];
...
const requiredCss = ['.status-card', '.room-scene', '.furniture', '.pet', '.pet-weimar', '.pet-sprite', 'data-weather', '.prototype-note', 'data-hungry', 'data-action', 'data-mood-state', '.mini-map', '.mini-map-art', '.map-dot', '.map-overlay', '.large-map', '.map-room'];
...
function runWithStorage(initialValue, label, options = {}) {
  ...
  assert(document.queries['.map-dot']?.length === 4, `${label}: mini map dots missing`);
  assert(document.queries['.map-room']?.length === 4, `${label}: large map rooms missing`);
...
function createDocument() {
  const queries = {};
  const byId = Object.fromEntries(['status-bars', 'mini-map', 'map-overlay', 'large-map', 'map-close', 'room-scene', 'room-title', 'pet', 'narration', 'world-chip', 'window-view', 'weather-layer', 'interaction-note', 'reset-save'].map((id) => [id, createElement(id, queries)]));
...
function registerGeneratedNodes(htmlText, queries) {
  if (htmlText.includes('map-dot')) {
    queries['.map-dot'] = Array.from({ length: count(htmlText, 'class="map-dot') }, () => ({}));
  }
