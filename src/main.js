...
const ROOMS = {
  ...
  kitchen: {
    ...
    furniture: [
      ...
      { id: 'dish', label: '餐盘', x: 52, y: 52, w: 8, h: 6, action: 'feed' },
    ],
  },
  bedroom: {
    label: '卧室',
...
function sanitizeSave(candidate) {
  ...
  const pets = candidatePets.map((pet, index) => {
    ...
    return {
      ...
      actionHistory: Array.isArray(pet?.actionHistory) ? pet.actionHistory.slice(-10).filter((item) => typeof item === 'string') : [],
...
function handleFurnitureClick(item) {
  ...
  activePet.actionHistory = [...(activePet.actionHistory ?? []), item.action].slice(-10);
  renderStatusBars();
  renderPetState();
  save();
}
